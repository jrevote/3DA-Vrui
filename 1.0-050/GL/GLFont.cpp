...
void GLFont::uploadStringTexture(const char* string,const GLFont::Color& stringBackgroundColor,const GLFont::Color& stringForegroundColor,GLsizei stringWidth,GLsizei textureWidth) const
	...
	/* Convert the string colors to 8-bit RGB: */
	GLColor<GLubyte,3> bg=stringBackgroundColor;
	GLColor<GLubyte,3> fg=stringForegroundColor;
	
	/* Create an RGB texture image of appropriate size: */
	GLColor<GLubyte,3>* image=new GLColor<GLubyte,3>[imageWidth*imageHeight];
	GLColor<GLubyte,3>* iPtr=image;
	...
	glPixelStorei(GL_UNPACK_SKIP_ROWS,0);
	glPixelStorei(GL_UNPACK_ALIGNMENT,1);
	glTexImage2D(GL_TEXTURE_2D,0,GL_RGB8,textureWidth,textureHeight,0,GL_RGB,GL_UNSIGNED_BYTE,0);
	if(antialiasing)
		{
		static GLfloat kernel[3]={0.25,0.5,0.25};
		// static GLfloat kernel[5]={0.125,0.2,0.35,0.2,0.125};
		glConvolutionParameteri(GL_SEPARABLE_2D,GL_CONVOLUTION_BORDER_MODE,GL_REPLICATE_BORDER);
		glSeparableFilter2D(GL_SEPARABLE_2D,GL_RGB8,3,3,GL_LUMINANCE,GL_FLOAT,kernel,kernel);
		glEnable(GL_SEPARABLE_2D);
