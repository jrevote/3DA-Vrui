...
class ValueCoder<Vrui::VRWindow::WindowType>
	...
	static Vrui::VRWindow::WindowType decode(const char* start,const char* end,const char** decodeEnd =0)
		...
		else if(end-start>=16&&strncasecmp(start,"SplitViewportStereo",18)==0)
			{
			if(decodeEnd!=0)
				*decodeEnd=start+18;
...
VRWindow::VRWindow(const char* windowName,const Misc::ConfigurationFileSection& configFileSection,VruiState* sVruiState,InputDeviceAdapterMouse* sMouseAdapter)
	...
	/* Check if the window's screen size should be defined on the X display's real size: */
	if(configFileSection.retrieveValue<bool>("./autoScreenSize",false))
		...
		vruiState->displaySize*=newSize/oldSize;
	...
	if(panningViewport)
		{
		/* Query the size of the display containing this window: */
		WindowPos rootPos=getRootWindowPos();
		for(int i=0;i<2;++i)
			displaySize[i]=rootPos.size[i];
		for(int i=0;i<2;++i)
			{
		...
		if(navigate)
			...
			vruiState->displayCenter=windowCenter;
			vruiState->displaySize=vruiState->displaySize*windowSize/screenSize;
			}
		}
	else
		{
	...
	/* Hide mouse cursor if the mouse is not used as an input device: */
	if(mouseAdapter==0)
		hideCursor();
	...
	if(windowType==AUTOSTEREOSCOPIC_STEREO)
		{
...
Scalar* VRWindow::getWindowCenterPos(Scalar windowCenterPos[2]) const
	...
	if(windowType!=SPLITVIEWPORT_STEREO&&panningViewport)
		{
		windowCenterPos[0]=(Scalar(getWindowOrigin()[0]+getWindowSize()[0]/2)+Scalar(0.5))*screens[0]->getWidth()/Scalar(displaySize[0]);
		windowCenterPos[1]=(Scalar(displaySize[1]-getWindowOrigin()[1]-getWindowSize()[1]/2)-Scalar(0.5))*screens[0]->getHeight()/Scalar(displaySize[1]);
		}
	else
		{
		...
		case ConfigureNotify:
			...
			if(panningViewport)
				...
				if(navigate)
					...
					/* Update the display center and size: */
					vruiState->displayCenter=newCenter;
					vruiState->displaySize=vruiState->displaySize*newSize/oldSize;
					}
...
void VRWindow::draw(void)
	...
	switch(windowType)
		...
		case AUTOSTEREOSCOPIC_STEREO:
			...
			for(int i=0;i<2;++i)
				{
				asTileSize[i]=GLWindow::getWindowSize()[i]/3;
				asTileTexCoord[i]=float(asTileSize[i])/float(asTextureSize[i]);
				asQuadSize[i]=asTileSize[i]*3;
				}
			...
			for(int zoneIndex=0;zoneIndex<9;++zoneIndex)
				...
				Point eyePos=asEye;
				eyePos+=asViewZoneOffsetVector*Scalar(zoneIndex-4);
	...
	if(saveScreenshot)
		...
		Images::writeImageFile(image,Misc::createNumberedFileName("VruiScreenshot.png",4,numberedFileName));
		#else
		/* Save the image buffer as a PPM file: */
		Images::writeImageFile(image,Misc::createNumberedFileName("VruiScreenshot.ppm",4,numberedFileName));
		#endif
