...
void DesktopDeviceNavigationTool::setButtonState(int buttonIndex,bool newButtonState)
	{
	if(buttonIndex==factory->navigationButtonIndex)
		{
		...
		if(newButtonState)
			...
			if(activate())
				...
				preScale=NavTrackerState::translateFromOriginTo(factory->navigationCenter);
				rotation=NavTrackerState::identity;
				zoom=Scalar(1);
				postScale=NavTrackerState::translateToOriginFrom(factory->navigationCenter);
...
void DesktopDeviceNavigationTool::frame(void)
	...
	if(isActive())
		...
		if(translation!=Vector::zero||scaledRotationAxis!=Vector::zero||deltaZoom[0]!=Scalar(0))
			requestUpdate();
		}
	else
		{
		...
		if(translation!=Vector::zero||scaledRotationAxis!=Vector::zero)
			requestUpdate();
		}
	}
