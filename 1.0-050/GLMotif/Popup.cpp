...
Vector Popup::calcNaturalSize(void) const
	...
	if(title!=0)
		...
		result[1]+=titleSize[1];
		}
	
	result[1]+=titleSpacing;
...
ZRange Popup::calcZRange(void) const
	...
	ZRange zRange=Container::calcZRange();
	
	/* Check the title widget: */
	if(title!=0)
		{
		ZRange titleZRange=title->calcZRange();
		if(zRange.first>titleZRange.first)
			zRange.first=titleZRange.first;
		if(zRange.second<titleZRange.second)
			zRange.second=titleZRange.second;
		}
	...
	if(child!=0)
		...
		if(zRange.first>childZRange.first)
			zRange.first=childZRange.first;
		if(zRange.second<childZRange.second)
			zRange.second=childZRange.second;
		}
	
	/* Adjust the minimum z value to accomodate the popup's back side: */
	zRange.first-=getBorderWidth();
...
void Popup::resize(const Box& newExterior)
	...
	/* Resize the title separator box: */
	titleSeparatorBox=getInterior();
	titleSeparatorBox.origin[0]+=marginWidth;
	titleSeparatorBox.size[0]-=2.0f*marginWidth;
	titleSeparatorBox.origin[1]+=titleSeparatorBox.size[1]-marginWidth-titleHeight-titleSpacing;
	titleSeparatorBox.size[1]=titleSpacing;
...
void Popup::draw(GLContextData& contextData) const
	...
	glVertex(titleSeparatorBox.getCorner(0));
	glVertex(titleSeparatorBox.getCorner(2));
	if(title!=0)
		{
		...
		glVertex(titleSeparatorBox.getCorner(3));
	...
	if(title!=0)
		glVertex(title->getExterior().getCorner(3));
	glVertex(titleSeparatorBox.getCorner(3));
	glVertex(titleSeparatorBox.getCorner(1));
	if(child!=0)
		{
