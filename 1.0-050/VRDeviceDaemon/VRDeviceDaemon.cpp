...
int main(int argc,char* argv[])
	...
	char* configFileName=SYSVRDEVICEDAEMONCONFIGFILENAME;
	...
	while(true)
		...
		try
			{
			configFile=new Misc::ConfigurationFile(configFileName);
			}
		...
		/* Set current section to name of current machine: */
		const char* hostname=getenv("HOSTNAME");
		configFile->setCurrentSection(hostname!=0?hostname:"localhost");
