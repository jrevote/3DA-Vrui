...
Copyright (c) 2006-2007 Braden Pellett, Oliver Kreylos
...
void HIDDevice::setupButtonAndAxisMaps ( Misc::ConfigurationFile& configFile )
{
	...
	for ( CFIndex i ( 0 ) ; i < CFArrayGetCount ( elements ) ; i++ )
	{
		...
		else if ( usagePage == 0x01 && usage >= 0x30 && usage <= 0x39 )
		{
			...
			converter . negMin = center ;
			converter . negMax = min ;
			converter . negRange = center - min ;
			converter . posMin = center ;
			converter . posMax = max ;
			converter . posRange = max - center ;
		}
		#ifdef VERBOSE
		else
		{
			...
			pos != axisCookies . end ( ) ; ++pos, ++index )
	{
		...
		try
		{
			AxisSettings absAxisSettings ( configFile.retrieveValue < AxisSettings > ( absAxisSettingsTag ) ) ;
			AxisConverter& converter ( absAxisMap [ pos -> second ] . converter ) ;
			converter . negMin = absAxisSettings . center - absAxisSettings . flat ;
			converter . negMax = absAxisSettings . minValue ;
			converter . negRange = converter . negMin - converter . negMax ;
			converter . posMin = absAxisSettings . center + absAxisSettings . flat ;
			converter . posMax = absAxisSettings . maxValue ;
			converter . posRange = converter . posMax - converter . posMin ;
		}
		catch ( Misc::ConfigurationFileBase::TagNotFoundError& )
		{ }
		#ifdef VERBOSE
			AxisConverter& converter ( absAxisMap [ pos -> second ] . converter ) ;
			printf ( "HIDDevice:    %-5d 0x%-4X 0x%-4X    %-+8.2f %-+8.2f %-8.2f   %-+8.2f %-+8.2f %-8.2f\n",
						index, pos -> first, pos -> second, 
						converter . negMax, converter . negMin, converter . negRange, 
						converter . posMin, converter . posMax, converter . posRange ) ;
		#endif
	}
}
