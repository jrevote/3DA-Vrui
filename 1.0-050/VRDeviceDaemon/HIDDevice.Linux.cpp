...
HIDDevice::HIDDevice(VRDevice::Factory* sFactory,VRDeviceManager* sDeviceManager,Misc::ConfigurationFile& configFile)
	...
	if(absAxisMap!=0)
		...
		for(int i=0;i<=ABS_MAX;++i)
			if(absAxisMap[i]>=0)
				...
				axisSettings.minValue=float(absAxisConf.minimum);
				axisSettings.maxValue=float(absAxisConf.maximum);
				axisSettings.center=(axisSettings.minValue+axisSettings.maxValue)*0.5f;
				axisSettings.flat=float(absAxisConf.flat);
				...
				axisSettings=configFile.retrieveValue<AxisSettings>(axisSettingsTag,axisSettings);
				
				#ifdef VERBOSE
				printf("Axis %2d: [%7.2f, %7.2f], %7.2f, %7.2f\n",absAxisMap[i],axisSettings.minValue,axisSettings.maxValue,axisSettings.center,axisSettings.flat);
				fflush(stdout);
				#endif
				
				/* Initialize converter: */
				AxisConverter& converter=axisConverters[absAxisMap[i]];
				converter.negMin=axisSettings.center-axisSettings.flat;
				converter.negMax=axisSettings.minValue;
				converter.negRange=converter.negMin-converter.negMax;
				converter.posMin=axisSettings.center+axisSettings.flat;
				converter.posMax=axisSettings.maxValue;
				converter.posRange=converter.posMax-converter.posMin;
				}
		}
	...
	if(relAxisMap!=0)
		...
		for(int i=0;i<=REL_MAX;++i)
			if(relAxisMap[i]>=0)
				{
				/* Create dummy configuration for this axis: */
				AxisSettings axisSettings;
				axisSettings.minValue=-1.0f;
				axisSettings.maxValue=1.0f;
				axisSettings.center=0.0f;
				axisSettings.flat=0.1f;
				...
				#ifdef VERBOSE
				printf("Axis %2d: [%7.2f, %7.2f], %7.2f, %7.2f\n",relAxisMap[i],axisSettings.minValue,axisSettings.maxValue,axisSettings.center,axisSettings.flat);
				fflush(stdout);
				#endif
