...
class MutexCond
	...
	void wait(const Lock&) // Waits on condition variable when lock is already established
		{
		pthread_cond_wait(&cond,&mutex);
		}
	bool timedWait(const Lock&,const Misc::Time& abstime) // Waits on condition variable when lock is already established; returns true if signal occurred; returns false if time expires
		{
