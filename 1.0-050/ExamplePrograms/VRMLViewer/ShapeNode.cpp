...
void ShapeNode::glRenderAction(GLContextData& contextData) const
	{
	if(appearance!=0)
		appearance->setGLState(contextData);
	
	if(geometry!=0)
		{
		/* Render the geometry node: */
		geometry->glRenderAction(contextData);
		}
	
	if(appearance!=0)
		appearance->resetGLState(contextData);
	}
